...
#ifndef _INCLUDED_to_json_unordered_map_hpp
#define _INCLUDED_to_json_unordered_map_hpp
...
namespace rapidjson
{
	// if it is an stl unordered_map that uses stl strings as keys, store it as
	// a normal json object
	template <typename WriterType, typename ValueType>
	void write(WriterType &                                       w,
	...
	void write(WriterType &                                   w,
	           const std::unordered_map<KeyType, ValueType> & values)
	{
		writeSequence(w, values);
	}
} // namespace rapidjson
#endif //_INCLUDED_to_json_unordered_map_hpp
